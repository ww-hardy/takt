...
final class StorageManager: StorageManaging, @unchecked Sendable {
  ...
  init() {
    ...
    // TAKT: Datenordner "wertwandler-takt" — Migration aus dem TAKT-Erbe
    // ("TAKT", sandbox-Container via StoragePathMigrator) beim ersten Start.
    let legacyBase = appSupport.appendingPathComponent("TAKT", isDirectory: true)
