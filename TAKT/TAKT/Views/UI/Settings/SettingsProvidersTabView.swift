  ...
  private var connectionHealthSection: some View {
    SettingsSection(
      ...
      VStack(alignment: .leading, spacing: 14) {
        ...
        case .local:
          VStack(alignment: .leading, spacing: 14) {
            if viewModel.localEngine == .llamaCpp {
              LlamaCppConfigurationView(
                configuration: $viewModel.llamaCppConfiguration,
                showsTitle: false
              )
            }
            LocalLLMTestView(
...
struct LocalModelUpgradeSheet: View {
  ...
  init(
    ...
    _llamaCppConfiguration = State(initialValue: LlamaCppConfiguration.load())
  }
  ...
  var body: some View {
    ScrollView(.vertical, showsIndicators: true) {
      VStack(alignment: .leading, spacing: 24) {
        ...
        if selectedEngine == .llamaCpp {
          ...
          .onChange(of: llamaCppConfiguration) { _, configuration in
            configuration.save()
            candidateBaseURL = configuration.baseURL
        ...
        LocalLLMTestView(
          ...
          showInputs: selectedEngine != .llamaCpp,
    ...
    .onChange(of: selectedEngine) { _, newEngine in
      if newEngine == .llamaCpp {
        llamaCppConfiguration = LlamaCppConfiguration.load()
        candidateBaseURL = llamaCppConfiguration.baseURL
        candidateModelId = "qwen3-vl-4b"
      } else {
