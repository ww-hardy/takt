...
final class ProvidersSettingsViewModel: ObservableObject {
  ...
  @Published var llamaCppConfiguration: LlamaCppConfiguration {
    didSet {
      guard oldValue != llamaCppConfiguration else { return }
      llamaCppConfiguration.save()
      if localEngine == .llamaCpp {
        localBaseURL = llamaCppConfiguration.baseURL
        localTestBaseURL = llamaCppConfiguration.baseURL
        localModelId = "qwen3-vl-4b"
        localTestModelID = "qwen3-vl-4b"
      }
    }
  }
  ...
  init() {
    ...
    localEngine = engine
    localBaseURL =
      UserDefaults.standard.string(forKey: "llmLocalBaseURL") ?? engine.defaultBaseURL
  ...
  var routingProviders: [CompactProviderInfo] {
    // TAKT: kein TAKT-Backend (Cloud) — Privacy-Versprechen.
  ...
  func isProviderConfigured(_ providerId: LLMProviderID) -> Bool {
    switch providerId {
    case .dayflow:
      return false  // TAKT-Backend gibt es in TAKT nicht mehr
    case .gemini:
  ...
  func statusText(for providerId: LLMProviderID) -> String? {
    ...
    case .local:
      ...
      let displayModel = localModelId.isEmpty
        ? LocalModelPreferences.defaultModelId(for: localEngine) : localModelId
      let truncatedModel =
